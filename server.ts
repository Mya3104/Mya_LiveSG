...
import { HDBResaleService, normalizeTownName, TOWN_MAPPING } from './src/services/hdbResaleService.ts';
import { OneMapService } from './src/services/oneMapService.ts';
...
// Mint / Refresh token (POST https://www.onemap.gov.sg/api/auth/post/getToken)
...
app.get('/api/onemap/reverse-geocode', async (req, res) => {
  try {
    ...
    // Handle either ?location=1.3,103.8 or ?latitude=1.3&longitude=103.8
...
app.get('/api/onemap/route', async (req, res) => {
  try {
    ...
    // Support start=1.320981,103.844150 or startLat / startLng
    if (req.query.start) {
    ...
    // Support end=1.326762,103.8559 or endLat / endLng
    if (req.query.end) {
...
app.post('/api/recommend', async (req, res) => {
  try {
    ...
    // Step A: Fetch top towns HDB live stats if available to feed the ranking model
    let hdbLiveStatsMap: Record<string, any> = {};
    try {
    ...
    // Step C: If Gemini API is available and query is non-empty, enrich top results with bespoke AI insight
...
// 5. AI Deep Estate Analysis & Transition Plan
app.post('/api/ai/deep-analysis', async (req, res) => {
  try {
    ...
    // Fetch official HDB stats for factual grounding
    let hdbStatsSummary = '';
    try {
...
// 6. AI Community Forum Sentiment Synthesizer
